...
# - 修复：CSV 回退 404/网络异常不再中断（软失败返回 None）
# - 显式 auto_adjust=False（yfinance FutureWarning）
# - 标量提取统一 .to_numpy()[-1].item()，避免未来 pandas 警告
...
def gold_spot():
    ...
    if s is None:
        try:
            ...
            pass
    return (float(s.to_numpy()[-1].item()) if s is not None and len(s) > 0 else None), s
...
def etf_score():
    ...
    win = min(10, n)
    x = np.arange(win)
    y = s.iloc[-win:].to_numpy().astype(float)
...
def main():
    ...
    tips_d = fred_series("DFII10", days=365*2)
    ...
    if tips_d is not None and len(tips_d)>6:
        a = tips_d.to_numpy()
        tips_wow = float(a[-1].item() - a[-6].item())

    dxy_30 = None
    if dxy_d is not None and len(dxy_d) >= 30:
        a = dxy_d.to_numpy()
        dxy_30 = (a[-1].item()/a[-30].item() - 1.0) * 100.0
    ...
    if tnx_df is not None and not tnx_df.empty and len(tnx_df["Close"].dropna())>6:
        close = tnx_df["Close"].dropna().to_numpy()
        tnx_wow = float(close[-1].item() - close[-6].item())
