...
#
# 变更摘要：
# - 新增 VAL_GAP_PCT（估值偏离 ±阈值 → ±20分）
# - ETF 自适应窗口：优先使用 ETF_LONG_D/ETF_SHORT_D，不足则尝试候选 "63:21,34:13,21:8"，再不足则自动 (auto)
# - 新增 DEBUG=1：在消息尾部打印数据源与行数（ETF/拍卖/央行YoY）和所选窗口
# - CSV 解析增强：支持 BTC_ETF_CSV_DATE_FIELD / BTC_ETF_CSV_FLOW_FIELD 精确指定 CSV 列名
# - 月频重采样统一 "ME"
import os, sys, html, traceback, io, json, math, re
from datetime import datetime, timedelta
import requests, pandas as pd, yfinance as yf, numpy as np
from dateutil import tz

TIMEOUT=25
UA={"User-Agent":"LiquidityMonitorBot/1.5.3"}
...
# ------------- ETF helpers ---------------
def read_csv_generic(url):
    r=requests.get(url, headers=UA, timeout=TIMEOUT); r.raise_for_status()
    df=pd.read_csv(io.StringIO(r.text))
    # 优先环境变量指定列
    date_field = env("BTC_ETF_CSV_DATE_FIELD")
    flow_field = env("BTC_ETF_CSV_FLOW_FIELD")
    if date_field and date_field in df.columns and flow_field and flow_field in df.columns:
        c1, c2 = date_field, flow_field
    else:
    ...
    out=df[[c1,c2]].rename(columns={c1:"Date", c2:"NetFlowUSD"})
    out["Date"]=pd.to_datetime(out["Date"], errors="coerce")
    out["NetFlowUSD"]=pd.to_numeric(out["NetFlowUSD"], errors="coerce")
    return out.dropna(subset=["Date","NetFlowUSD"]).sort_values("Date")

def etf_daily_series():
    # 1) API (SoSoValue)
    source=""
    if env("BTC_ETF_API_URL"):
        ...
        if method=="POST":
            ...
            try:
                r=requests.post(env("BTC_ETF_API_URL"), headers=headers, json=jbody, timeout=TIMEOUT); r.raise_for_status()
                ...
                if rows:
                    ...
                    if dcol and fcol:
                        ...
                        source="API"
                        return df.dropna(subset=["Date","NetFlowUSD"]).sort_values("Date"), source
            except Exception:
                pass
        else:
            # GET
            try:
                r=requests.get(env("BTC_ETF_API_URL"), headers=headers, timeout=TIMEOUT); r.raise_for_status()
                j=r.json(); data=j.get("data",{}); rows=data.get("list",[])
                if rows:
                    df=pd.DataFrame(rows)
                    dcol = "date" if "date" in df.columns else next((c for c in df.columns if str(c).lower() in ("date","day","datetime")), None)
    ...
    try:
        ...
        df["Date"]=pd.to_datetime(df["Date"], errors="coerce")
        df["NetFlowUSD"]=pd.to_numeric(df["NetFlowUSD"], errors="coerce")
        return df.dropna().sort_values("Date"), "Farside"
    except Exception: 
        return None, ""
...
def main():
    ...
    # ETF 源
    etf_df_all, etf_src0 = etf_daily_series()
    etf_rows = 0 if etf_df_all is None else len(etf_df_all)
    if etf_df_all is not None:
        Lpref = int(env("ETF_LONG_D","63"))
        Spref = int(env("ETF_SHORT_D","21"))
        L,S, tag = choose_etf_windows(etf_rows, Lpref, Spref)
        if L is None:
        ...
        L=S=None; tag=""
    # 周聚合
    etf_weekly, etf_src = (None, etf_src0 or "N/A")
    try:
    ...
    if DEBUG:
        diag = []
        diag.append(f"ETF源={etf_src0 or 'N/A'} 行数={etf_rows} 窗口={('N/A' if (L is None or S is None) else f'{L}/{S} {tag}')}")
        lines.append("<i>诊断：</i> " + " | ".join(diag))
