...
def get_dxy_change(days=30):
    ...
    params={"series_id":"DTWEXBGS","api_key":api_key,"file_type":"json",
            "observation_start":(datetime.utcnow()-timedelta(days=days+365)).strftime("%Y-%m-%d")}
    try:
        r=requests.get(url, params=params, headers=UA, timeout=TIMEOUT); r.raise_for_status()
        obs=r.json().get("observations",[])
...
def cb_yoy_pct_from_wgc():
    try:
        ...
        dcol=None
        for c in df.columns:
            if str(c).lower() in ("month","date","period"): dcol=c; break
        if dcol is None:
            for c in df.columns:
                try:
        ...
        if vcol is None:
            ...
            if nums: vcol=nums[0]
        if not dcol or not vcol: return None, "WGC columns"
        df=df[[dcol,vcol]].copy()
        df[dcol]=pd.to_datetime(df[dcol], errors="coerce")
        df=df.dropna().sort_values(dcol)
        latest=df.iloc[-1]
        prev_year_month=(latest[dcol]-pd.DateOffset(years=1)).strftime("%Y-%m")
        df["ym"]=df[dcol].dt.strftime("%Y-%m")
        v_now=float(latest[vcol])
        v_prev=float(df.loc[df["ym"]==prev_year_month, vcol].iloc[-1])
