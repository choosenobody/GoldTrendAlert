...
#
# 新增：
# - 信号 1-4 显示为 0–100 分（仅显示层面，内部加权计算不变）
# - 央行购金 YoY 支持自定义滚动窗口 CB_YOY_ROLL_M（默认 12），若样本不足自动退到 6
# - 其余沿用 v1.5.0（回归截距基准 REG、消息排版等）
...
TIMEOUT=25
UA={"User-Agent":"LiquidityMonitorBot/1.5.1"}
...
# ---- helpers omitted comments for brevity (same as v1.5.0) ----
...
def get_weekly_btc_etf_flows():
    # Simple weekly sum path: prefer API → CSV → Farside
    def csv_url(url):
        try:
            r=requests.get(url, headers=UA, timeout=TIMEOUT); r.raise_for_status()
            df=pd.read_csv(io.StringIO(r.text))
            c1=next((c for c in df.columns if str(c).lower() in ("date","day","datetime")), None)
            c2=next((c for c in df.columns if str(c).lower() in ("netflowusd","flowusd","net_flow_usd","netflow","flow","totalnetinflow")), None)
            if not (c1 and c2): return None
            df=df[[c1,c2]].rename(columns={c1:"Date", c2:"NetFlowUSD"})
            return df
        except Exception: return None

    url_api=env("BTC_ETF_API_URL")
    df=None
    if url_api:
        try:
            ...
            if hdr:
                try:
                    import json; headers.update(json.loads(hdr))
                except: pass
            if method=="POST":
                body=env("BTC_ETF_API_BODY","{}")
                try: j=json.loads(body)
                except: j={"type":"us-btc-spot"}
                r=requests.post(url_api, headers=headers, json=j, timeout=TIMEOUT)
            else:
                r=requests.get(url_api, headers=headers, timeout=TIMEOUT)
            r.raise_for_status()
            j=r.json(); data=j.get("data", j)
            rows=data.get("list") if isinstance(data,dict) else (data if isinstance(data,list) else None)
            if rows:
                df=pd.DataFrame(rows)
                dcol= "date" if "date" in df.columns else next((c for c in df.columns if str(c).lower() in ("date","day","datetime")), None)
                fcol= "totalNetInflow" if "totalNetInflow" in df.columns else next((c for c in df.columns if "inflow" in str(c).lower() or str(c).lower().endswith("flow")), None)
    ...
    if df is None and env("BTC_ETF_FLOWS_CSV_URL"):
        df=csv_url(env("BTC_ETF_FLOWS_CSV_URL"))
    if df is None:
        try:
            r=requests.get(env("FARSIDE_ALLDATA_URL","https://farside.co.uk/bitcoin-etf-flow-all-data/"), headers=UA, timeout=TIMEOUT); r.raise_for_status()
            tables=pd.read_html(r.text)
            cand=None; best=0
            for t in tables:
                if t.shape[1] < 2: continue
                tt=t.copy()
                try:
                    pd.to_datetime(tt.iloc[:,0], errors="raise")
                    if len(tt)>best: cand=tt; best=len(tt)
                except: pass
            if cand is not None:
                df=cand[[cand.columns[0], cand.columns[-1]]].copy()
                df.columns=["Date","NetFlowUSD"]
    ...
    if df is None: return None, "ETF N/A"
    ...
    return val, ("API(POST)" if url_api else "CSV/Farside")
...
def main():
    ...
    lines.append(f"3）ETF 净流向趋势（21/63 EMA＋斜率）：{etf_note} → <b>{s3_100}</b>")
